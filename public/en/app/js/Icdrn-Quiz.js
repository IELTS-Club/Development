...
function userLevel(userScore, quizState) {
  ...
  if (userScore <= 25) {   // Beginner 0-15 - 15 - Each(5) - 5-5-5
    ...
    quizState.level = "(B1) - Beginner-1";

  } else if (userScore <= 50) {
    
    quizState.level = "(B2) - Beginner-2";

  } else if (userScore <= 75) {
    
    quizState.level = "(B3) - Beginner-3";

  } else if (userScore <= 105) {   // Elemantry 16-24 - 9 - Each(10) - 3-3-3
    
    quizState.level = "(E1) - Elementary-1";

  } else if (userScore <= 135) {
    
    quizState.level = "(E2) - Elementary-2";

  } else if (userScore <= 165) {
    
    quizState.level = "(E3) - Elementary-3";

  } else if (userScore <= 210) {   // Pre-inter 25-32 - 8 - Each(15) - 3 - 3 - 2
    
    quizState.level = "(PI-1) - Pre-Intermadiate-1";

  } else if (userScore <= 255) {
    
    quizState.level = "(PI-2) - Pre-Intermadiate-2";

  } else if (userScore <= 285) {
    
    quizState.level = "(PI-3) - Pre-Intermadiate-3";

  } else if (userScore <= 345) {   // inter 33-39 - 7 - Each(20) - 3 - 2 - 2
    
    quizState.level = "(I-1) - Intermadiate-2";

  } else if (userScore <= 385) {
    
    quizState.level = "(I-2) - Intermadiate-2";

  } else if (userScore <= 425) {
    
    quizState.level = "(I-3) - Intermadiate-3";

  } else if (userScore <= 475) {   // Upper-Inter 40-45 - 6 - Each(25) - 2 - 2 - 2
    
    quizState.level = "(UI-1) - Upper-Intermadiate-1";

  } else if (userScore <= 525) {
    
    quizState.level = "(UI-2) - Upper-Intermadiate-2";

  } else if (userScore <= 575) {
    
    quizState.level = "(UI-3) - Upper-Intermadiate-3";

  } else if (userScore <= 635) {   // Advancer 46-50 - 5 - Each(30) - 2 - 2 - 1
    
    quizState.level = "(A-1) - Advance-1";

  } else if (userScore <= 695) {
    
    quizState.level = "(A-2) - Advance-2";

  } else if (userScore <= 725) {
    
    quizState.level = "(A-3) - Advance-3";
